...
match.data <- propensity.master  %>%
  select(gender, age, no_of_BrMs, volume, localization_of_BrMs,
         adj.tx2, KPS_group, adj.RTx2, primary_tumor.Rx, dose_of_RTx, UICC2,
...
prop.matching <- matchit(adj.tx2 ~ gender + age + no_of_BrMs + volume + localization_of_BrMs + KPS_group + extracran.met + adj.RTx2 + 
                           primary_tumor.Rx + dose_of_RTx + UICC2,
...
summary.tab.prop <- match.data.final %>% select(adj.tx2, gender, age, no_of_BrMs, volume, localization_of_BrMs, KPS_group, extracran.met, adj.RTx2, 
                                                primary_tumor.Rx, dose_of_RTx, UICC2)
...
m.out02 <- matchit(adj.tx2 ~ gender + age +  no_of_BrMs + primary_tumor.Rx2 + pre.tx + adj.RTx2 +
                    dose_of_RTx + KPS_group + UICC2 + extracran.met + volume + censoring + survival, data = m.out02,
